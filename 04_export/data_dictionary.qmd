...
detect_csv_delimiter <- function(file_path) {
  ...
  comma_count <- str_count(first_line, ",")
  semicolon_count <- str_count(first_line, ";")
  if (comma_count > semicolon_count) "," else ";"
}

# Conditional data loading based on params$type
if (data_type == "source") {
  # Load raw source data
  source_file <- config::get("data_1cho_enrollments_file_path")
  delim <- detect_csv_delimiter(source_file)
  df <- readr::read_delim(source_file, delim = delim, show_col_types = FALSE)
  data_description <- paste("Bronbestand:", basename(source_file))
  # All variables are from source
  original_vars <- names(df)
} else {
  # Load prepared data (end of pipeline)
  # Paths relative to project root (execute_dir = getwd() in render function)
  ...
  data_description <- "Prepared data na pipeline"
...
# Add descriptions from source metadata where available (only for end-of-pipeline)
if (data_type == "end") {
  stats <- stats |>
    left_join(
      source_metadata |>
...
`r if(data_type == "end") paste0("**Herkomst:** Bron (1CHO): ", sum(stats$Herkomst == "Bron"), " | Toegevoegd (pipeline): ", sum(stats$Herkomst == "Toegevoegd"))`
